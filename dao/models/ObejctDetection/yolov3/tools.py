...
def iou_score(bboxes_a, bboxes_b):
    """
    Function: 计算bboxes_a和bboxes_b的IOU
        bbox_1 : [B*N, 4] = [x1, y1, x2, y2]
        bbox_2 : [B*N, 4] = [x1, y1, x2, y2]

    :return [number_of_a, number_of_b]
    """
    tl = torch.max(bboxes_a[:, :2], bboxes_b[:, :2])    # top left
    br = torch.min(bboxes_a[:, 2:], bboxes_b[:, 2:])    # bottom right
    area_a = torch.prod(bboxes_a[:, 2:] - bboxes_a[:, :2], 1)
    area_b = torch.prod(bboxes_b[:, 2:] - bboxes_b[:, :2], 1)

    en = (tl < br).type(tl.type()).prod(dim=1)      # 判断此bbox面积是否大于0
    area_i = torch.prod(br - tl, 1) * en  # * ((tl < br).all()) # 计算交集面积
